...
DepthFlow DepthMake(
    ...
    vec3 intersect = vec3(depth.center + camera.gluv, 1.0)
        - vec3(camera.position.xy, 0.0) * (1.0/(1.0 - rel_steady)) * float(depth.glued);

    // The quality of the parallax effect is how tiny the steps are
    // Optimization: Low quality overshoot, high quality reverse
    float quality = (1.0 / mix(200.0, 2000.0, depth.quality));
    float probe   = (1.0 / mix( 50.0,  120.0, depth.quality));
...
void main() {
    GetCamera(iCamera);
    DepthFlow iDepth;
    iDepth.isometric = iDepthIsometric;
    iDepth.dolly     = iDepthDolly;
    iDepth.zoom      = iDepthZoom;
    iDepth.offset    = iDepthOffset;
    iDepth.height    = iDepthHeight;
    iDepth.focus     = iDepthFocus;
    iDepth.center    = iDepthCenter;
    iDepth.steady    = iDepthSteady;
    iDepth.origin    = iDepthOrigin;
    iDepth.quality   = iQuality;
    iDepth.glued     = true;
    iDepth.value     = 0.0;
    iDepth.gluv      = vec2(0.0);
    iDepth.oob       = false;
    ...
    if (iLensIntensity > 0.0) {
        ...
        fragColor.rgb = (color / float(iLensQuality));
    }
    ...
    // Depth of Field (Mutually exclusive with lens distortion)
        ...
        for (float angle=0.0; angle<TAU; angle+=TAU/float(iBlurDirections)) {
            for (float walk=1.0/float(iBlurQuality); walk<=1.001; walk+=1.0/float(iBlurQuality)) {
                vec2 displacement = vec2(cos(angle), sin(angle)) * walk * intensity;
                color += gtexture(image, depthflow.gluv + displacement, true);
            }
        }
        fragColor = color / float(iBlurDirections*iBlurQuality);
    }
