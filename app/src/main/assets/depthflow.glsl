...
DepthFlow DepthMake(
    ...
    camera.origin += vec3(depth.origin, 0.0);
    ...
    depth.steep = depth.derivative * angle(depth.normal, vec3(0.0, 0.0, 1.0));
...
#define GetDepthFlow(name) \
    DepthFlow name; \
    { \
        name.isometric = iDepthIsometric; \
        name.dolly     = iDepthDolly; \
        name.zoom      = iDepthZoom; \
        name.offset    = iDepthOffset; \
        name.height    = iDepthHeight; \
        name.focus     = iDepthFocus; \
        name.center    = iDepthCenter; \
        name.steady    = iDepthSteady; \
        name.origin    = iDepthOrigin; \
