...
float smoothlerp(float a, float b, float difference) {
    ...
    float offset = difference*t*(1.0 - t)/2.0;
...
float smin(float a, float b) {return smoothlerp(a, b,  1.0);}
float smax(float a, float b) {return smoothlerp(a, b, -1.0);}
...
vec2 agluv2gluv(vec2 agluv) {return agluv * vec2(iAspectRatio, 1.0);}
vec2 gluv2agluv(vec2 gluv) {return gluv / vec2(iAspectRatio, 1.0);}
...
vec2 astuv2stuv(vec2 astuv) {
    return vec2(astuv.x*iAspectRatio + (1.0 - iAspectRatio)/2.0, astuv.y);
}
vec2 stuv2astuv(vec2 stuv) {
    return vec2((stuv.x - (1.0 - iAspectRatio)/2.0)/iAspectRatio, stuv.y);
}

// Apply a virtual GL_MIRRORED_REPEAT to a AGLUV coordinate
vec2 agluv_mirrored_repeat(vec2 agluv) {
    return vec2(
        triangle_wave(agluv.x, 4.0),
        triangle_wave(agluv.y, 4.0)
    );
}
...
vec2 gluv_mirrored_repeat(vec2 gluv) {
    return vec2(
        iWantAspect * triangle_wave(gluv.x, 4.0*iWantAspect),
...
bool astuv_oob(vec2 astuv) {
    return (astuv.x<0.0)||(astuv.x>1.0)||(astuv.y<0.0)||(astuv.y>1.0);
}
bool stuv_oob(vec2 stuv) {
    return astuv_oob(stuv2astuv(stuv));
}
bool agluv_oob(vec2 agluv) {
    return (agluv.x<-1.0)||(agluv.x>1.0)||(agluv.y<-1.0)||(agluv.y>1.0);
}
...
vec4 gtexture(sampler2D image, vec2 gluv) {
    ivec2 resolution = textureSize(image, 0);
    vec2 scale = vec2(float(resolution.y)/float(resolution.x), 1.0);
...
float _sdLine(vec3 origin, vec3 A, vec3 B, bool segment) {
    ...
    if (segment) t = clamp(t, 0.0, 1.0);
    return length(shortest - direction*t);
}

float sdLine(vec2 origin, vec2 p1, vec2 p2) {
    return _sdLine(vec3(origin, 0.0), vec3(p1, 0.0), vec3(p2, 0.0), false);}
...
float sdLineSegment(vec2 origin, vec2 p1, vec2 p2) {
    return _sdLine(vec3(origin, 0.0), vec3(p1, 0.0), vec3(p2, 0.0), true);}
...
float sdBox(vec3 origin, vec3 point, vec3 size) {
    vec3 d = abs(origin - point) - size/2.0;
...
float sdSmoothUnion(float a, float b, float width) {
    float k = clamp(0.5 + 0.5*(b-a)/width, 0.0, 1.0);
    return mix(b, a, k) - width*k*(1.0 - k);
}
...
vec3 hsv2rgb(vec3 hsv) {
    ...
    int sector = int(floor(6.0 * h / TAU));
    if (sector == 0) rgb = vec3(c, x, 0.0);
    else if (sector == 1) rgb = vec3(x, c, 0.0);
    else if (sector == 2) rgb = vec3(0.0, c, x);
    else if (sector == 3) rgb = vec3(0.0, x, c);
    else if (sector == 4) rgb = vec3(x, 0.0, c);
